...
// Verificar campos vacíos
   ...
   */
  const { nombre_alumno, email_alumno, curso_alumno, DNI, fecha_registro, celular, direccion } = req.body;
  try {
    // Realizar la inserción en la base de datos
    const query =
      "INSERT INTO estudiantes (nombre_alumno, email_alumno, DNI, celular, direccion, curso_alumno, created_at) VALUES (? ,?, ? ,?, ?, ?, ?)";
...
app.post("/procesar-formulario2", (req, res) => {
  console.log(req.body);
  const { nombre_alumno, email_alumno, curso_alumno, DNI, celular, direccion } = req.body;
  try {
    const query =
      "INSERT INTO estudiantes (nombre_alumno, email_alumno, DNI, celular, direccion, curso_alumno, created_at) VALUES (?,? ,? ,?, ?, ? ,?)";
    connection.query(
      query,
      [nombre_alumno, email_alumno, DNI, celular, direccion, curso_alumno,  new Date()],
      (error, result) => {
        if (error) {
          console.error("Error al insertar en la base de datos: ", error);
          res.send("Error al procesar el formulario");
...
app.listen(PORT, () => {
  console.log(`Servidor escuchando en http://54.236.250.120:${PORT}`);
});
